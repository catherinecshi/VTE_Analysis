...
class Betasort:
    ...
    def __init__(self, n_stimuli, rat, day, tau=0.05, xi=0.95):
        ...
        self.R_history = [self.R.copy()]
        self.N_history = [self.N.copy()]
    
    def choose(self, chosen, unchosen, vte=False, zlength=0):
        """
        ...
        #noise = chosen_reward_hist / (unchosen_reward_hist + chosen_reward_hist)
        uncertainty = self.get_uncertainty_ROC(chosen, unchosen) # higher -> more certain
        reward_history = (self.R[chosen] + self.R[unchosen]) / (self.R[chosen] + self.R[unchosen] + self.N[chosen] + self.N[unchosen])
        noise = reward_history * uncertainty
    ...
    def choose_VTE(self, chosen, unchosen):
        certainty_ROC = self.get_uncertainty_ROC(chosen, unchosen) # higher -> more certain
        uncertainty_chosen = self.get_uncertainty_stimulus(chosen) # higher -> more uncertain
        uncertainty_unchosen = self.get_uncertainty_stimulus(unchosen) # higher -> more uncertain
        reward_history = (self.R[chosen] + self.R[unchosen]) / (self.R[chosen] + self.R[unchosen] + self.N[chosen] + self.N[unchosen])
        noise = reward_history * certainty_ROC
        vte_uncertainty = (1 - certainty_ROC) * (uncertainty_chosen * 10 + uncertainty_unchosen * 10)
        ...
        chosen_idx = np.argmax(X) # choose stimulus with largest value
        return available_stimuli[chosen_idx], vte_occurred
    ...
    def update(self, chosen, unchosen, reward, probability, threshold):
        ...
        if reward == 1 and probability < threshold: # if correct trial + choice did not match data
            ...
            self.U[chosen] = self.U[chosen] + V[chosen]
            self.L[unchosen] = self.L[unchosen] + V[unchosen]
            self.U = self.U + V
            self.L = self.L + (1 - V)
            ...
            # shifts chosen up, unchosen down
            self.U[chosen] = self.U[chosen] + (V[chosen] * 0.25)# 
            self.L[unchosen] = self.L[unchosen] + (V[unchosen] * 0.25) #
            self.U = self.U + V
            self.L = self.L + (1 - V)
        else:
    ...
    def update_ti(self, chosen, unchosen, reward):
        ...
        if reward == 1: # correct trial + choice matched data
            self.R[unchosen] = self.R[unchosen] + 1
            self.R[chosen] = self.R[chosen] + 1
    ...
    def get_uncertainty_stimulus(self, stimulus_idx):
        ...
        # return iqr
        #q25 = stats.beta.ppf(0.25, a, b)
        #q75 = stats.beta.ppf(0.75, a, b)
        #return q75 - q25
        
        # return entropy
        #return np.log(beta(a, b)) - (a - 1) * digamma(a) - (b - 1) * digamma(b) + (a + b - 2) * digamma(a + b)
        
        # return uncertainty by comparing it to uncertainty of other stimuli
        #stimulus_evidence = a + b
        #all_evidence = self.U + self.L

        #evidence_rank = np.sum(all_evidence <= stimulus_evidence) / self.n_stimuli
        #variance = ci_upper - ci_lower
        
        #return variance / (evidence_rank + 0.1)
    ...
    def get_uncertainty_ROC(self, chosen_idx, other_idx, n_samples=100):
        """
        ...
        uncertainty = uncertainty * n_samples / 2
        
        return uncertainty
    
    def get_all_ROC_uncertainties(self):
        """get probabilistic uncertainties for all adjacent pairs of stimulus"""
        return np.array([self.get_uncertainty_ROC(i, i+1) for i in range(self.n_stimuli-1)])
