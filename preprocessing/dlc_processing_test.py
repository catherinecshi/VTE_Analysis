...
if __name__ == "__main__":
    for rat in paths.vte_data.iterdir():
        if ".DS_Store" in rat.name:
            continue
        
        filtered_info = []
        
        # determine which part to track rats on
        if any(rat.name == rat_ID for rat_ID in settings.IMPLANTED_RATS):
            track_part = "greenLED"
        else:
            track_part = "nose"
        
        settings.update_rat(rat.name)
        logger.info(f"starting processing for {rat.name}")
        
        # make folder if it doesn't exist for rat already
        rat_folder = paths.cleaned_dlc / remote.module
        if not os.path.exists(rat_folder):
            os.mkdir(rat_folder)
        
        # retrieve data
        data_folder = paths.vte_data / rat.name / remote.module / "Day1"
        
        try:
            DLC_path = next(data_folder.glob("*DLC*.csv"))
            DLC_df = data_processing.process_dlc_data(DLC_path)
            SS_path = next(data_folder.glob("*stateScriptLog*"))
            SS_log = data_processing.process_statescript_log(SS_path)
            timestamps_path = next(data_folder.glob("*videoTimeStamps*"))
        ...
        try:
            ...
            if len(DLC_df) - len(timestamps) == 1:
                ...
                logger.error(f"unequal dlc frames {len(DLC_df)} and timestmaps {len(timestamps)} for {rat}")
                continue
        ...
        try:
            ...
            logger.debug(f"{e} with SS for {rat}")
        ...
        save_path_coords = rat_folder / f"{rat.name}_test_coordinates.csv"
        coords_df.to_csv(save_path_coords, index=False)
        
        check_too_filtered(DLC_df, x, y, track_part)
