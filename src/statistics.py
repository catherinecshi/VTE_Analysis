...
def plot_one_way_anova_bar(data_groups, group_labels=None, title=None, xlabel=None, ylabel=None):
    ...
    """
    # draw significance bars if any significant results
    if stats_results["p_value"] < 0.05:
        max_height = max(means) + max(sems)
        bar_height = max_height * 1.1
        
        x_left, x_right = 0, len(data_groups) - 1
        ax.plot([x_left, x_left, x_right, x_right],
                [bar_height, bar_height + max_height * 0.05, bar_height + max_height * 0.05, bar_height],
                color="black", linewidth=1)
        
        # add star
        ax.text((x_left + x_right) / 2, bar_height + max_height * 0.05, "*", ha="center", va="bottom")
    """
