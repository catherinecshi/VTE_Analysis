...
def calculate_trial_type_agreement(data):
    """
    Calculate the percentage of agreement between vte_occurred and model_vte_occurred
    ...
    for rat_name, df in data.items():
        ...
        df['agreement'] = (df['vte_occurred'] == df['model_vte_occurred']).astype(int)
...
def calculate_trial_type_vte_agreement(data):
    """
    Calculate the percentage of agreement between vte_occurred and model_vte_occurred
    for each trial type, but only for trials where both rat and model show VTE (both = 1).
    ...
    for rat_name, df in data.items():
        # Filter to only trials where both rat and model show VTE
        vte_trials = df[(df['vte_occurred'] == 1) & (df['model_vte_occurred'] == 1)]
...
def calculate_vte_percentages(data):
    ...
    Returns:
        DataFrame with columns: rat, day, vte_occurred_pct, model_vte_occurred_pct
    """
    results = []
    ...
    for rat_name, df in data.items():
        ...
        day_stats = df.groupby('day').agg({
            ...
            'model_vte_occurred': lambda x: (x == 1).mean() * 100
        }).reset_index()
    ...
    if results:
        ...
        combined_df.rename(columns={
            ...
            'model_vte_occurred': 'model_vte_occurred_pct'
        }, inplace=True)
        return combined_df
    else:
        return pd.DataFrame()
...
def plot_trial_type_agreement(trial_agreements, save_path=None):
    ...
    ax.set_title('Model-Rat VTE Agreement by Trial Type')
    ax.set_ylim(0, 100)
...
def plot_trial_type_vte_agreement(trial_vte_agreements, save_path=None):
    ...
    ax.set_title('Percentage of Trials with Both Model and Rat VTE by Trial Type')
    ax.set_ylim(0, 100)
...
def plot_vte_scatter(vte_percentages, save_path=None, sample_size=50):
    """
    ...
    scatter = ax.scatter(vte_percentages['vte_occurred_pct'], 
                        vte_percentages['model_vte_occurred_pct'],
                        alpha=0.6, s=50)
    
    # Randomly sample data for correlation calculation
    np.random.seed(42)  # For reproducible results
    if len(vte_percentages) > sample_size:
        sampled_data = vte_percentages.sample(n=sample_size)
    else:
        sampled_data = vte_percentages
    
    # Calculate Pearson correlation on sampled data
    correlation, p_value = stats.pearsonr(sampled_data['vte_occurred_pct'], 
                                         sampled_data['model_vte_occurred_pct'])
    
    ax.set_xlabel('Rat VTE Percentage (%)')
    ax.set_ylabel('Model VTE Percentage (%)')
    ax.set_title(f'Model vs Rat VTE Percentages by Day\nr = {correlation:.3f}, p = {p_value:.4f} (n={len(sampled_data)})')
...
def plot_vte_scatter_log(vte_percentages, save_path=None, sample_size=50):
    """
    ...
    non_zero_data = vte_percentages[
        ...
        (vte_percentages['model_vte_occurred_pct'] > 0)
    ]
    ...
    scatter = ax.scatter(non_zero_data['vte_occurred_pct'], 
                        non_zero_data['model_vte_occurred_pct'],
                        alpha=0.6, s=50)
    
    # Randomly sample non-zero data for correlation calculation
    np.random.seed(42)  # For reproducible results
    if len(non_zero_data) > sample_size:
        sampled_data = non_zero_data.sample(n=sample_size)
    else:
        sampled_data = non_zero_data
    
    # Calculate Pearson correlation for sampled non-zero data
    correlation, p_value = stats.pearsonr(sampled_data['vte_occurred_pct'], 
                                         sampled_data['model_vte_occurred_pct'])
    
    ax.set_xlabel('Rat VTE Percentage (%) - Log Scale')
    ax.set_ylabel('Model VTE Percentage (%) - Log Scale')
    ax.set_title(f'Model vs Rat VTE Percentages by Day (Log Scale)\nr = {correlation:.3f}, p = {p_value:.4f} (n={len(sampled_data)})')
    ax.set_xscale('log')
    ax.set_yscale('log')
    
    # Add diagonal line for perfect agreement
    min_val = min(non_zero_data['vte_occurred_pct'].min(), non_zero_data['model_vte_occurred_pct'].min())
    max_val = max(non_zero_data['vte_occurred_pct'].max(), non_zero_data['model_vte_occurred_pct'].max())
...
def main():
    ...
    plot_trial_type_agreement(trial_agreements, 'trial_type_agreement.png')
    
    # Plot 2: Trial type concurrent VTE
    plot_trial_type_vte_agreement(trial_vte_agreements, 'trial_type_vte_agreement.png')
    
    # Plot 3: VTE percentage scatter
    plot_vte_scatter(vte_percentages, 'vte_percentage_scatter.png')
    
    # Plot 4: VTE percentage scatter with log scale
    plot_vte_scatter_log(vte_percentages, 'vte_percentage_scatter_log.png')
    
    print("\nPlots saved as 'trial_type_agreement.png', 'trial_type_vte_agreement.png', 'vte_percentage_scatter.png', and 'vte_percentage_scatter_log.png'")
